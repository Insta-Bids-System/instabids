#!/usr/bin/env python
"""
COIA Memory Working Test - Simple Version
Test what actually works in the COIA memory integration without Unicode issues
"""
...
async def test_coia_working_functionality():
    ...
    try:
        ...
        print("[OK] System created successfully")
        ...
        if result1:
            ...
            print(f"[OK] Response generated: {len(result1['messages'][-1].content)} characters")
            print(f"[OK] Company extracted: {company}")
            print(f"[OK] Profile fields: {len(profile)}")
            ...
            print(f"[OK] Key fields extracted: {len(extracted_fields)}/{len(key_fields)} - {extracted_fields}")

            test1_success = len(extracted_fields) >= 2
            print(f"TEST 1 {'PASS' if test1_success else 'FAIL'}: Profile extraction working")
        else:
            print("[FAIL] TEST 1 FAIL: No response generated")
            test1_success = False
        ...
        if result2:
            ...
            print(f"[OK] Mode detected: {current_mode}")
            print(f"[OK] Response generated: {len(result2['messages'][-1].content)} characters")

            test2_success = current_mode == "bid_card_search"
            print(f"TEST 2 {'PASS' if test2_success else 'FAIL'}: Bid card search mode triggered")
        else:
            print("[FAIL] TEST 2 FAIL: No response generated")
            test2_success = False
        ...
        if success_rate >= 0.8:
            ...
            print("PARTIAL: Some COIA functionality working")
            status = "partial"
        else:
...
async def main():
    ...
    if status == "excellent":
        print("SUCCESS: COIA system is working excellently!")
        print("Ready for enhanced memory integration.")
    elif status == "good":
        print("SUCCESS: COIA system is working well!")
        print("Basic functionality confirmed, ready for production testing.")
    elif status == "partial":
        print("PARTIAL: COIA has partial functionality working.")
        print("Focus on fixing failing components.")
    else:
        print("FAIL: COIA needs debugging before memory integration.")
