...
This uses the real images you just provided
"""
...
async def upload_local_image_to_leonardo(image_path, description):
    """Upload a local image file to Leonardo and get the ID"""
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                print(f"[INFO] Reading local image: {image_path}")
                with open(image_path, 'rb') as f:
                    image_data = f.read()
                print(f"[SUCCESS] Read {len(image_data)} bytes")
                ...
                if isinstance(upload_fields, dict):
                    ...
                    print(f"[ERROR] upload_fields is not a dict after parsing")
                    return None
...
async def generate_backyard_transformation(current_image_id, turf_image_id):
    ...
    # Transformation prompt specifically for YOUR backyard
    generation_data = {
        "prompt": "Transform this backyard by replacing all dead and patchy grass areas with beautiful, lush green artificial turf that looks perfectly maintained like a golf course. The artificial turf should be vibrant emerald green, uniform, pristine and well-manicured. Keep the white soccer goal in the exact same position. Preserve all existing structures including the building, trees, bushes, green bin, landscaping borders, and mulch areas exactly as they are. Professional landscape transformation, high quality, photorealistic.",
        "negative_prompt": "blurry, low quality, distorted, unrealistic, cartoon, painting, sketch, different layout, moved structures, brown patches, dead grass",
        "modelId": "6bef9f1b-29cb-40c7-b9df-32b51c1f67d3",  # Leonardo Phoenix
        "width": 1024,
        "height": 768,
        "num_images": 1,
        "init_image_id": current_image_id,
        "init_strength": 0.3,  # Keep structure, just change the grass texture
        "guidance_scale": 7,
        "seed": None,
        "controlnets": None  # Leonardo Phoenix v2 will handle the transformation
    }
    
    try:
        async with aiohttp.ClientSession() as session:
            print("[INFO] Starting your backyard transformation...")
            print("   YOUR actual backyard -> artificial turf transformation")
...
async def main():
    ...
    print("=" * 70)
    print("[TRANSFORM] YOUR ACTUAL BACKYARD -> ARTIFICIAL TURF TRANSFORMATION")
    print("=" * 70)
    print("Using YOUR REAL IMAGES:")
    print("[OK] Your backyard with patchy grass and soccer goal")
    print("[OK] Your ideal artificial turf reference")
    print()
    
    # First, save your images locally from the clipboard/message
    print("[NOTE] Please ensure your backyard images are saved as:")
    print("  1. C:\\Users\\Not John Or Justin\\Documents\\instabids\\backyard_current.jpg")
    print("  2. C:\\Users\\Not John Or Justin\\Documents\\instabids\\turf_ideal.jpg")
    print()
    
    backyard_path = "C:\\Users\\Not John Or Justin\\Documents\\instabids\\backyard_current.jpg"
    turf_path = "C:\\Users\\Not John Or Justin\\Documents\\instabids\\turf_ideal.jpg"
    
    # Check if files exist
    if not os.path.exists(backyard_path):
        print(f"[ERROR] Cannot find your backyard image at: {backyard_path}")
        print("Please save your backyard image to this location first")
        return
        
    if not os.path.exists(turf_path):
        print(f"[WARNING] Cannot find turf reference at: {turf_path}")
        print("Continuing without style reference...")
        turf_path = None
    
    # Step 1: Upload your current backyard
    print("STEP 1: Uploading YOUR current backyard image...")
    current_id = await upload_local_image_to_leonardo(backyard_path, "YOUR current backyard")
    if not current_id:
        print("[ERROR] Failed to upload current backyard")
        return
    
    # Step 2: Upload your ideal turf reference (optional)
    turf_id = None
    if turf_path:
        print("\nSTEP 2: Uploading YOUR ideal turf reference...")
        turf_id = await upload_local_image_to_leonardo(turf_path, "YOUR ideal artificial turf")
        if not turf_id:
            print("[WARNING] Failed to upload turf reference, continuing without it")
    ...
    if result_url and result_url.startswith("http"):
        ...
        print("   - YOUR patchy grass replaced with lush artificial turf")
        print("   - All YOUR structures and landscaping maintained")
        print()
        print("[NEXT] You can now:")
        print("   - View the transformation in your browser")
        print("   - Save it to show contractors")
        print("   - Use this for your project planning")
...
if __name__ == "__main__":
    print("[IMPORTANT] First, save your images to:")
    print("  1. C:\\Users\\Not John Or Justin\\Documents\\instabids\\backyard_current.jpg")
    print("  2. C:\\Users\\Not John Or Justin\\Documents\\instabids\\turf_ideal.jpg")
    print()
    input("Press Enter when you've saved the images...")
    asyncio.run(main())