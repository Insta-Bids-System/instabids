...
class MultiProjectMemoryStore:
    ...
    async def save_user_memory(self, user_id: str, memory_type: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if existing.data:
                # Update existing
                result = self.db.client.table('user_memories').update(memory_data).eq(
                    'user_id', user_id
                ).eq('memory_type', memory_type).execute()
            else:
                # Create new
                memory_data['created_at'] = datetime.utcnow().isoformat()
                result = self.db.client.table('user_memories').insert(memory_data).execute()
    ...
    async def save_project_context(self, user_id: str, project_id: str, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if existing.data:
                # Update existing
                result = self.db.client.table('project_contexts').update(context_data).eq(
                    'user_id', user_id
                ).eq('project_id', project_id).execute()
            else:
                # Create new
                result = self.db.client.table('project_contexts').insert(context_data).execute()
    ...
    async def save_project_summary(self, user_id: str, project_id: str, summary: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if existing.data:
                # Update existing
                result = self.db.client.table('project_summaries').update(summary_data).eq(
                    'user_id', user_id
                ).eq('project_id', project_id).execute()
            else:
                # Create new
                result = self.db.client.table('project_summaries').insert(summary_data).execute()
