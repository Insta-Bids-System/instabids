...
from .campaign_orchestrator import OutreachCampaignOrchestrator, OutreachChannel
...
class EnhancedCampaignOrchestrator:
    ...
    async def _select_tier_contractors(self,
        ...
        try:
            if tier == 1:
                # Select from internal contractors
                # First try contractors table, then fallback to high-score contractor_leads
                try:
                    # First try contractors table
                    ...
                    for c in contractors:
                        c['tier'] = 1
                        c['company_name'] = c.get('business_name', 'Unknown')
                except:
                    # Fallback: use high-score potential_contractors as Tier 1
    ...
    def _determine_optimal_channels(self,
        ...
        if urgency in ['emergency', 'urgent']:
            # For urgent projects, use all channels
            return ['email', 'sms', 'website_form', 'phone']
        elif urgency == 'standard':
            # Standard projects: email and website forms
            return ['email', 'website_form']
        else:
            # Flexible projects: primarily email
            return ['email']
