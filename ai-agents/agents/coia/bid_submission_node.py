...
async def _create_contractor_account(profile: dict, contractor_lead_id: Optional[str]) -> dict:
    """Create contractor account and transfer data"""
    ...
    try:
        ...
        # 2. Transfer data from contractor_leads to contractors table
        # 3. Send welcome email with temporary password
        # 4. Set up initial permissions and access
        
        logger.info(f"Creating account for {profile.get('company_name')}")
        
        return {
            "success": True,
            "contractor_id": f"CONTRACTOR-{datetime.now().strftime('%Y%m%d%H%M%S')}",
