"""
OpenAI O3-Based CoIA (Contractor Onboarding & Intelligence Agent)
Uses OpenAI's O3 model for advanced reasoning and natural language understanding
"""
...
class OpenAIO3CoIA:
    """OpenAI O3-Based CoIA using advanced reasoning for contractor onboarding"""

    def __init__(self, api_key: str = None):
        """Initialize with OpenAI O3 for intelligent understanding"""
        ...
        logger.info("OpenAI O3 CoIA initialized")
        ...
        if not supabase_url or not supabase_key:
            ...
            logger.info("OpenAI O3 CoIA initialized with Supabase")
            ...
            if service_role_key:
                ...
                print(f"[O3 COIA INIT] Service role key loaded: {service_role_key[:20]}...")
            else:
                self.supabase_admin = None
                logger.warning("No service role key - auth operations will fail")
                print("[O3 COIA INIT] NO SERVICE ROLE KEY FOUND!")
        ...
        if self.google_api_key:
            ...
            print(f"[O3 COIA INIT] Google API Key loaded: {self.google_api_key[:20]}...")
        else:
            print("[O3 COIA INIT] NO GOOGLE API KEY FOUND!")

    async def process_message(self, session_id: str, user_message: str,
                            context: Optional[dict[str, Any]] = None) -> dict[str, Any]:
        """Process user message with O3's advanced reasoning"""

        print(f"[O3 COIA] Processing message: '{user_message}' for session: {session_id}")
...
Analyze this message and extract business information."""
        ...
        try:
            # Call OpenAI model (O3 not yet available, using GPT-4)
            response = self.client.chat.completions.create(
                model="gpt-4-turbo-preview",  # Using GPT-4 until O3 is available
                messages=[
            ...
            # Parse the O3 response
            understanding = json.loads(response.choices[0].message.content)
            print(f"[O3 UNDERSTANDING] {understanding}")

            return understanding

        except Exception as e:
            print(f"[O3 ERROR] Failed to understand message: {e}")
...
- Use the business name to create reasonable social media handles"""
        ...
        try:
            # Call O3 for intelligent research
            response = self.client.chat.completions.create(
            ...
            print("[O3 RESEARCH] O3 response received and parsed")
...
Return the updated data in the same JSON format with corrections applied."""
    ...
    async def _standard_conversation(self, state: CoIAConversationState, user_message: str) -> dict[str, Any]:
        ...
        response = """Hi! I'm your contractor onboarding assistant powered by OpenAI's O3 advanced reasoning.
...
# Global instance
openai_o3_coia: Optional[OpenAIO3CoIA] = None

def initialize_openai_o3_coia(api_key: str = None) -> OpenAIO3CoIA:
    """Initialize global OpenAI O3 CoIA"""
    global openai_o3_coia
    openai_o3_coia = OpenAIO3CoIA(api_key)
    return openai_o3_coia

def get_openai_o3_coia() -> Optional[OpenAIO3CoIA]:
    """Get global OpenAI O3 CoIA"""
    return openai_o3_coia
