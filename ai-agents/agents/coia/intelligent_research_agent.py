...
class IntelligentResearchCoIA:
    ...
    async def process_message(self, session_id: str, user_message: str, 
        ...
        if not state:
            print(f"[INTELLIGENT COIA] Creating new session with persistent memory")
            state = await persistent_coia_state_manager.create_session(session_id)
        else:
            print(f"[INTELLIGENT COIA] Found existing session in persistent memory")
        ...
        try:
            ...
            if business_info and not state.research_completed:
                print(f"[INTELLIGENT COIA] Triggering intelligent research")
                ...
                if returning_contractor_id:
                    ...
                    print(f"[INTELLIGENT COIA] New contractor - proceeding with research")
                ...
                print(f"[INTELLIGENT COIA] Handling confirmation")
                # Handle confirmation
                return await self._handle_confirmation(state, user_message)
            else:
                print(f"[INTELLIGENT COIA] Standard conversation")
    ...
    async def _intelligent_research(self, state: CoIAConversationState, 
        ...
        if self.google_api_key:
            print(f"[INTELLIGENT RESEARCH] Calling Google Maps API...")
            ...
            print(f"[INTELLIGENT RESEARCH] No Google API key - skipping search")
        ...
        print(f"[INTELLIGENT RESEARCH] Claude Opus 4 response received")
        ...
        try:
            ...
            print(f"[INTELLIGENT RESEARCH] Found real data:")
    ...
    def _generate_confirmation_response(self, research_data: RealBusinessData) -> str:
        ...
        response_parts = [
            f"Great! I found your business online and here's what I discovered:",
            "",
    ...
    async def _handle_confirmation(self, state: CoIAConversationState, user_message: str) -> Dict[str, Any]:
        ...
        if confirmed:
            print(f"[HANDLE CONFIRMATION] Creating contractor profile...")
...
2. **Use your business email**: {state.research_data.email}
3. **Choose "Contractor" account type**
4. **Your profile will be automatically linked** to this research

Your Contractor ID: {contractor_id}

Once you sign up, you'll have full access to your contractor dashboard and can start receiving project invitations from homeowners in your area!"""
    ...
    async def _create_contractor_profile(self, state: CoIAConversationState) -> Optional[str]:
        """Create contractor profile in database with real researched data AND create Supabase auth account"""
        ...
        if not self.supabase or not hasattr(state, 'research_data'):
            print(f"[CREATE PROFILE] Missing requirements - returning None")
            return None
        ...
        try:
            ...
            # Create contractor lead with real data
            contractor_data = {
                'source': 'manual',
                'company_name': research.company_name,
                'email': research.email or None,
                'phone': research.phone or None,
                'website': research.website or None,
                'specialties': research.services,
                'years_in_business': research.years_experience if research.years_experience and research.years_experience > 0 else None,
                'lead_status': 'qualified',
                'contractor_size': 'small_business',
                'service_zip_codes': research.service_areas,
                'lead_score': 95,  # High score for AI-verified research
                'data_completeness': 90 if research.email and research.phone else 70,
                'raw_data': {
            ...
            print(f"[CREATE PROFILE] Inserting contractor data...")
            result = self.supabase.table('contractor_leads').insert(contractor_data).execute()
            
            print(f"[CREATE PROFILE] Insert result: {result}")
            
            if result.data:
                contractor_id = result.data[0]['id']
                print(f"[CREATE PROFILE] Success! Contractor ID: {contractor_id}")
                logger.info(f"Created contractor profile with real data: {contractor_id}")
                return contractor_id
            else:
                print(f"[CREATE PROFILE] Failed - no data returned")
...
I'll use advanced AI to research your business online and create your complete InstaBids profile with real, verified information. What's your business name?"""
    ...
    async def _handle_returning_contractor(self, state: CoIAConversationState, contractor_id: str, business_name: str) -> Dict[str, Any]:
        ...
        if conversation_history:
            ...
            response_parts = [
                ...
                "",
                f"I found your previous profile in our system:",
    ...
    def _error_response(self, state: CoIAConversationState, custom_message: str = None) -> Dict[str, Any]:
        """Error response"""
