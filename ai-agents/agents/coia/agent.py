...
from .state import CoIAConversationState, ContractorProfile, coia_state_manager
...
class CoIAAgent:
    ...
    def __init__(self, api_key: str):
        ...
        if not supabase_url or not supabase_anon_key:
            ...
            # Admin client for user creation (if service key available)
            if supabase_service_key:
                self.supabase_admin: Client = create_client(supabase_url, supabase_service_key)
                logger.info("CoIA Supabase clients initialized (including admin)")
            else:
                self.supabase_admin = None
                logger.warning("Service key not found - will create contractor profiles without auth accounts")
...
"""
    ...
    async def _create_contractor_profile(self, state: CoIAConversationState) -> Optional[str]:
        """Create complete contractor account (auth user + profile + contractor record)"""
        if not self.supabase:
            logger.warning("Supabase not initialized - cannot create contractor profile")
            return None

        try:
            # Convert CoIA profile data to create complete account
            profile = state.profile

            # Extract email from profile data (from AI research)
            contractor_email = getattr(profile, "email", None)
            company_name = getattr(profile, "company_name", None) or getattr(profile, "business_name", None)

            if not contractor_email:
                logger.error("No email found in profile - cannot create auth account")
                return None
            ...
            if self.supabase_admin:
                try:
                    ...
                    if profile_result.data:
                        logger.info(f"✅ Created profile record for user: {auth_user_id}")
                ...
                auth_user_id = str(uuid.uuid4())
                logger.warning("No admin client - using temp user_id")
            ...
            if result.data and len(result.data) > 0:
                ...
                logger.info(f"Email: {contractor_email}")
                logger.info(f"Auth User ID: {auth_user_id}")
                logger.info(f"Specialties: {contractor_data['specialties']}")

                # Store auth info in state for response
                state.auth_credentials = {
                    "email": contractor_email,
                    "password": temp_password,
                    "has_auth_account": bool(self.supabase_admin)
                }
