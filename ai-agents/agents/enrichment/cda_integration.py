...
from agents.enrichment.test_langchain_architecture import LangChainMCPArchitecture, EnrichedContractorData
...
class CDAwithLangChainEnrichment:
    ...
    def _smart_select_by_preferences(
        ...
        if not filtered_contractors:
            print(f"[CDAwithLangChainEnrichment] No exact matches, expanding criteria...")
...
async def test_cda_langchain_integration():
    ...
    print(f"\nTEST SCENARIO:")
    print(f"   Homeowner: 'I need lawn care but want a local company with employees, not just one guy'")
    print(f"   Project: Lawn care maintenance")
    print(f"   Location: Coconut Creek, FL 33442") 
    print(f"   Business Size Preference: LOCAL_BUSINESS_TEAMS")
    ...
    print(f"   [OK] CDA contractor discovery")
    print(f"   [OK] LangChain MCP enrichment agent")
    print(f"   [OK] Business size classification")
    print(f"   [OK] Service type detection")
    print(f"   [OK] Smart selection by preferences")
    print(f"   [OK] Complete enriched data output")
    ...
    print(f"   1. Connect LangChain agent to real MCP Playwright server")
    print(f"   2. Add proper Anthropic API key for agent intelligence") 
    print(f"   3. Test with real contractor websites")
    print(f"   4. Store enriched data in database")
    print(f"   5. Integrate with EAA for outreach campaigns")
