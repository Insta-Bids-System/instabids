"""
MCP Email Channel with Claude Integration
Uses Claude Opus 4 to write personalized emails for each contractor
"""
import os
import uuid
...
class MCPEmailChannelWithClaude:
    """Email channel that uses Claude to write personalized emails"""
    ...
    def __init__(self):
        ...
        if not self.anthropic_api_key:
            print("[MCPEmailChannel+Claude ERROR] ANTHROPIC_API_KEY not found in environment!")
        else:
            print(f"[MCPEmailChannel+Claude] Found API key: {self.anthropic_api_key[:10]}...")
        
        self.client = Anthropic(api_key=self.anthropic_api_key)
        self.sent_emails = []  # For testing verification
        print("[MCPEmailChannel+Claude] Initialized with Claude Opus 4 integration")
    
    def send_personalized_outreach(self, 
                                 contractor: Dict[str, Any], 
                                 bid_card_data: Dict[str, Any], 
                                 campaign_id: str) -> Dict[str, Any]:
        """Send personalized email using Claude to write content"""
        try:
            message_id = str(uuid.uuid4())[:8]
            
            # Use Claude to create personalized email
            ...
            print(f"[MCPEmailChannel+Claude] Sent Claude-written email to {contractor.get('company_name')}")
            return result
            
        except Exception as e:
            print(f"[MCPEmailChannel+Claude ERROR] Failed: {e}")
            return {'success': False, 'error': str(e)}
    
    def _create_claude_personalized_email(self,
                                        contractor: Dict[str, Any],
                                        bid_card_data: Dict[str, Any],
                                        message_id: str,
                                        campaign_id: str) -> Dict[str, Any]:
        """Use Claude to write a unique, personalized email"""
...
BODY: [your email body here]"""
        ...
        try:
            ...
            print(f"[Claude] Generated unique email for {company_name}")
            ...
            print(f"[Claude ERROR] Failed to generate email: {e}")
    ...
    def _convert_to_html(self, plain_text: str, cta_url: str, company_name: str, contractor_email: str) -> str:
        """Convert Claude's plain text to styled HTML"""
    ...
    def _send_via_mcp(self, to_email: str, email_data: Dict[str, Any], 
        ...
        try:
            print(f"[MCP] Would send email via mcp__instabids-email__send_email")
            print(f"  To: {to_email}")
            ...
            return {
                ...
                'claude_generated': email_data['company_personalization'].get('claude_generated', False),
                'unique_elements': {
