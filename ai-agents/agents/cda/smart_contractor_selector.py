...
class SmartContractorSelector:
    ...
    def select_best_contractors(
        ...
        try:
            ...
            logger.error(f"Error in smart contractor selection: {str(e)}")
            return {
                'success': False,
                'error': str(e),
    ...
    def _filter_viable_contractors(self, contractors: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for contractor in contractors:
            ...
            if rating >= self.min_rating and reviews >= self.min_reviews:
                viable.append(contractor)
            elif rating >= 4.5 and reviews >= 3:  # Exception for very high rated newer businesses
                viable.append(contractor)
            elif reviews >= 50:  # Exception for very established businesses
                viable.append(contractor)
