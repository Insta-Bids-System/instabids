...
from agents.cia.new_state import (
    GROUP_BIDDING_PROJECTS,
    INTENTION_SCORE_FACTORS,
    REQUIRED_FIELDS_MINIMAL,
    SERVICE_TYPE_KEYWORDS,
)
from agents.cia.prompts import get_conversation_prompt
...
class CustomerInterfaceAgent:
    ...
    async def handle_conversation(
            ...
            try:
                ...
                for idx, base64_image in enumerate(images):
                    try:
                        ...
                        # Store in photo_storage table
                        photo_record = {
                            "user_id": user_id,
                            "project_id": session_id,  # Use session_id as project identifier
                            "photo_data": clean_base64,
                            "mime_type": "image/jpeg",
                            "description": f"User uploaded photo {idx + 1}"
                        }

                        result = self.supabase.table("photo_storage").insert(photo_record).execute()

                        if result.data:
                            photo_id = result.data[0]["id"]
                            stored_image_ids.append(photo_id)
                            print(f"[CIA] Stored image {idx + 1} in database: {photo_id}")
                        else:
                            print(f"[CIA] Warning: Failed to store image {idx + 1}")

                    except Exception as e:
                        print(f"[CIA] Error storing image {idx + 1}: {e}")
                        continue

                # Store photo IDs in collected_info but keep original images for Claude
                if stored_image_ids:
                    state["collected_info"]["uploaded_photos"].extend([f"photo_id:{photo_id}" for photo_id in stored_image_ids])
                    print(f"[CIA] Successfully stored {len(stored_image_ids)} images in database")
                    # Keep original images for Claude API processing
                else:
                    print("[CIA] Warning: No images were stored successfully")

            except Exception as e:
                print(f"[CIA] Error in image storage process: {e}")
        ...
        try:
            ...
            print("[CIA] Saved conversation to database for JAA")
        except Exception as e:
            print(f"[CIA] ERROR: Could not save to database: {e}")
    ...
    async def _generate_claude_response(self, state: dict[str, Any]) -> str:
        ...
        print("[CIA] _generate_claude_response called")
...
"""
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                print("[CIA] Could not extract JSON from Claude response")
                return {}
