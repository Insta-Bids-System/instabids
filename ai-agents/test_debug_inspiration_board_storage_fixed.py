...
def test_inspiration_board_storage_debug():
    ...
    print(f"SUCCESS: Test image loaded: {len(image_data)} characters")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"SUCCESS: IRIS Response Status: 200")
            ...
            if 'workflow_questions' in result:
                ...
                if follow_response.status_code == 200:
                    ...
                    print(f"SUCCESS: Follow-up Response Status: 200")
                    ...
                    if 'storage_results' in follow_result:
                        ...
                        if 'inspiration_board' in storage:
                            ...
                            if not inspiration_result.get('success'):
                                print(f"ERROR: INSPIRATION BOARD FAILED:")
                    ...
                    print(f"ERROR: Follow-up request failed: {follow_response.status_code}")
                    print(f"Error: {follow_response.text}")
                    return False
            else:
                print(f"ERROR: No workflow questions in response")
                return False
        else:
            print(f"ERROR: IRIS request failed: {response.status_code}")
...
def verify_storage_results(user_id, session_id):
    ...
    try:
        ...
        # Query property photos (joined through properties)
        ...
        if inspiration_boards.data:
            ...
            inspiration_images = db.client.table("inspiration_images").select("*").eq("board_id", board_id).execute()
            print(f"Images in board: {len(inspiration_images.data)}")
            ...
            if inspiration_images.data:
                ...
                print(f"   ERROR: No images found in inspiration board {board_id}")
        else:
            print(f"   ERROR: No inspiration boards found for user {user_id}")
...
if __name__ == "__main__":
    print("DEBUG: IRIS Inspiration Board Storage Debug Test")
    print("=" * 50)
    ...
    print("=" * 50)
    if success:
        print("SUCCESS: INSPIRATION BOARD STORAGE: WORKING")
    else:
        print("FAILED: INSPIRATION BOARD STORAGE: STILL BROKEN")
