...
async def test_complete_workflow():
    ...
    print("="*70)
    print(f"Testing complete CIA -> JAA -> CDA -> EAA -> WFA pipeline")
    ...
    if confidence >= 100:
        print(f"   Status: HIGH CONFIDENCE - Expected to meet bid target")
    elif confidence >= 75:
        print(f"   Status: GOOD CONFIDENCE - Likely to meet bid target")
    else:
        print(f"   Status: LOW CONFIDENCE - May need additional contractors")
...
async def simulate_outreach_strategy_calculation(bid_card, contractors):
    ...
    if urgency in ['emergency', 'urgent']:
        ...
        multiplier = 1.5 if urgency == 'emergency' else 1.2
...
async def main():
    ...
    try:
        ...
        if result['workflow_completed']:
            ...
            print(f"Some steps failed or were skipped")
