"""
Iris Chat API - Using Claude Opus 4 for intelligent conversations
"""
...
from .image_generation import GenerateDreamSpaceRequest, generate_dream_space
...
async def iris_chat_working(request: IrisChatRequest):
    """
    Iris chat powered by Claude Opus 4 with image generation capabilities
    """
    try:
        ...
        response = await generate_claude_response(request.message, is_generation_request, request)

        return response

    except Exception as e:
        print(f"Error in Iris chat: {e}")
...
async def generate_claude_response(message: str, is_generation_request: bool, request: IrisChatRequest = None):
    """Generate response using Claude Opus 4"""

    # Create system prompt for Iris
    system_prompt = """You are Iris, an AI assistant specialized in helping homeowners with home improvement and renovation projects. You are knowledgeable, friendly, and creative.

Key capabilities:
- Help homeowners plan and visualize home improvement projects
- Generate AI images when users ask to "create", "generate", "show me", or want a "vision" of their space
- Provide practical advice about renovations, materials, costs, and design
- Ask clarifying questions to better understand their needs

When users ask for image generation (vision, create, generate, show me, etc.):
1. Acknowledge their request enthusiastically
2. Let them know you're creating their vision
3. Actually generate the image using your tools
4. Describe what you've created

Be conversational, helpful, and excited about their projects. Always aim to understand their style preferences and practical needs."""

    try:
        # Call Claude Opus 4
        response = claude.messages.create(
            model="claude-3-5-sonnet-20241022",  # Using latest Claude 3.5 Sonnet (best available)
            max_tokens=1000,
            temperature=0.7,
        ...
        # If this is an image generation request, actually generate the image
        if is_generation_request and request:
            try:
                print("[IRIS] Claude detected image generation request, creating vision...")
                ...
                generation_result = await generate_dream_space(generation_request)

                if generation_result.get("success"):
                    return {
                        "response": iris_response + "\n\nI've created your vision! The image has been generated and saved to your inspiration board.",
                        "image_generated": True,
                        "image_url": generation_result.get("generated_image_url"),
                        "generation_id": generation_result.get("generation_id")
                    }
                else:
                    return {
                        "response": iris_response + "\n\nI tried to create your vision, but encountered an issue with image generation. Let me help you in other ways!",
                        "image_generated": False
                    }

            except Exception as gen_error:
                print(f"[IRIS] Image generation error: {gen_error}")
                return {
                    "response": iris_response + "\n\nI'd love to create a vision for you, but I'm having some technical difficulties with image generation right now. Let me help you plan your project in other ways!",
                    "image_generated": False
                }

        # Regular response without image generation
        return {
            "response": iris_response,
            "image_generated": False
        }
        ...
        print(f"[IRIS] Claude API error: {e}")
        raise e
