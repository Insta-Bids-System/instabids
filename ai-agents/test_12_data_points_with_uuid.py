#!/usr/bin/env python3
"""
Test CIA → JAA Flow with 12 Data Points Extraction using valid UUID
"""
import asyncio
import json
import os
...
async def test_cia_collection():
    ...
    print(f"Using valid UUID for user: {user_id}")
    ...
    try:
        ...
        if session_id:
            ...
            if bid_card_number:
                ...
                print("="*80)
                print("\nThe new 12 data points extraction system is fully operational:")
                print("- CIA collects all InstaBids-focused data points")
                print("- JAA extracts and stores all data in bid cards")
                print("- Budget extraction is correct (not $4-$329)")
                print("- Group bidding potential identified")
                print("- Intention scoring working (9/10 for this test)")
            else:
