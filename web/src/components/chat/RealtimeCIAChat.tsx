...
import { Upload, Mic, MicOff, Volume2, VolumeX, Sparkles, Zap, Brain } from 'lucide-react'
import toast from 'react-hot-toast'
import { OpenAIRealtimeClient, AudioProcessor } from '@/services/openai-realtime'
...
Keep responses conversational and natural. Use casual language while maintaining professionalism.`,
  ...
  }, [audioMode, voiceEnabled, onSendMessage])
    ...
    if (files.length > 0) {
      for (const file of files) {
        try {
          ...
          if (realtimeClient.current) {
            realtimeClient.current.sendText(`User uploaded an image: ${file.name}. Image URL: ${uploadedFile.url}`)
          }
        } catch (error) {
  ...
  const getPersonalityColor = () => {
    switch (personality) {
      case 'friendly': return 'from-blue-400 to-blue-600'
      case 'professional': return 'from-gray-400 to-gray-600'
      case 'enthusiastic': return 'from-yellow-400 to-orange-500'
      case 'thoughtful': return 'from-purple-400 to-purple-600'
      case 'helpful': return 'from-green-400 to-green-600'
  ...
  const getPersonalityIcon = () => {
    switch (personality) {
      case 'enthusiastic': return <Zap className="w-4 h-4" />
