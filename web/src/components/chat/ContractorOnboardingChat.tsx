...
import {
  Send, Bot, User, Loader2, CheckCircle, TrendingUp, 
  Building2, MapPin, Award, Clock, DollarSign
} from 'lucide-react';
...
const ContractorOnboardingChat: React.FC<ContractorOnboardingChatProps> = ({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: "Welcome to InstaBids! I'm CoIA, your contractor success assistant. I'm here to help you get set up and start winning more projects.\n\nTo get started, what's your primary trade or specialty? (For example: General Contractor, Plumber, Electrician, HVAC, etc.)",
      timestamp: new Date(),
      stage: 'welcome'
    },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Call CoIA backend
      const response = await fetch('/api/contractor-chat/message', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to send message to CoIA');
      }
      ...
      setMessages(prev => prev.map(msg => 
        msg.id === assistantMessageId 
          ? { ...msg, content: mockResponse.response, isStreaming: false, stage: mockResponse.stage }
          : msg
      ));
  ...
  const generateMockCoIAResponse = (userInput: string, currentProgress: ProfileProgress) => {
    ...
    if (currentProgress.stage === 'welcome') {
      if (input.includes('general') || input.includes('contractor')) {
        ...
        return {
          response: "Perfect! Plumbing is always in high demand. We have plenty of plumbing projects from simple repairs to full bathroom renovations.\n\nHow many years of plumbing experience do you have?",
          stage: 'experience',
          profileUpdate: {
    ...
    <div className="flex flex-col h-[600px] bg-white/5 backdrop-blur-sm">
      ...
      <div className="p-4 border-b border-white/10">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm text-white/70">{Math.round(profileProgress.completeness * 100)}% Complete</span>
        </div>
