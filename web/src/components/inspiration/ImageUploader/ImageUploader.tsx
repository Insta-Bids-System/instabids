...
import { Upload, X, CheckCircle, AlertCircle, Home, Sparkles, Target } from 'lucide-react'
...
const ImageUploader: React.FC<ImageUploaderProps> = ({ 
  ...
  const [uploadCategory, setUploadCategory] = useState<'current' | 'inspiration' | 'vision'>(defaultCategory)
  ...
  const uploadFiles = async (uploads: UploadProgress[]) => {
    ...
    for (let i = 0; i < uploads.length; i++) {
      ...
      try {
        ...
        const tags = uploadCategory === 'current' ? ['current'] : 
                    uploadCategory === 'vision' ? ['vision'] : []
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => setUploadCategory('current')}
          ...
          <button
            onClick={() => setUploadCategory('vision')}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-all ${
          ...
          <div className="space-y-2">
            {uploadQueue.map((upload, index) => (
              ...
              >
                ...
                {/* Remove Button */}
                {(upload.status === 'pending' || upload.status === 'error' || upload.status === 'success') && (
                  <button
