import { Camera, Image, Link2, RefreshCw, Send, Sparkles, Upload, X } from "lucide-react";
...
Understanding your current challenges will help me suggest the best improvements.`;
  ...
  const _handleGenerateImage = async () => {
...
Please upload ${!idealImageId && !currentImageId ? "both images" : !idealImageId ? "an ideal inspiration image" : "a current space image"} to continue.`,
        timestamp: new Date(),
        suggestions: ["Upload ideal inspiration", "Upload current space", "How does this work?"],
      };
...
I'm adjusting the design based on: "${feedback}"`,
    ...
    try {
      const response = await fetch(
        "http://localhost:8008/api/image-generation/regenerate-with-feedback",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
