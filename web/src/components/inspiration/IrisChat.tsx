...
export default function IrisChat({
  ...
  const handleImageClick = (image: BoardImage) => {
    ...
    if (!selectedImages.find((img) => img.id === image.id)) {
      ...
      const imageType = image.tags.includes("current")
        ? "current space"
  ...
  }, [clickedImage, onImageProcessed, handleImageClick]);
  ...
  const simulateIrisResponse = (_userInput: string) => {
    ...
    setTimeout(() => {
      ...
      if (currentImages.length === 0) {
        response =
          "I notice you haven't selected your current space photo yet. Click on your current room photo so I know what we're working with.";
      } else if (inspirationImages.length === 0) {
        response =
          "Great! I can see your current space. Now click on some inspiration images to show me what style elements you like.";
      } else {
        // Extract elements from inspiration images
        const allFeatures = inspirationImages.flatMap((img) => img.ai_analysis?.key_features || []);
  ...
  const handleGeneration = () => {
    ...
    if (!currentImage || inspirationImages.length === 0) {
      const errorMessage: ChatMessage = {
        ...
        type: "assistant",
        message:
          "I need both your current space photo and at least one inspiration image to generate a vision. Please select them by clicking on the images.",
        timestamp: new Date(),
      };
    ...
    const generatingMessage: ChatMessage = {
      ...
      type: "assistant",
      message:
        "I'm generating your vision now! This will show your exact current space enhanced with the specific elements you requested...",
      timestamp: new Date(),
    };
          ...
          <div className="flex items-center gap-2 overflow-x-auto">
            ...
            {selectedImages.map((img) => (
              <div key={img.id} className="relative">
                ...
                <button
                  type="button"
                  onClick={() => setSelectedImages(selectedImages.filter((si) => si.id !== img.id))}
